...
import {
  ...
  pickTransferCountry, buildTripVisits, gateSceneFor,
...
/* ---------- §0-3 幕間宣言（にゅうこくしんさ／のりつぎ）の出し分け境界ケース ---------- */
test("gateSceneFor: パック本来の国・forward はentry宣言・辛口フレーバーなし", () => {
  assert.deepEqual(gateSceneFor(false, "forward"), { kind: "entry", showReverseFlavor: false });
});
test("gateSceneFor: のりつぎ国・forward はtransfer宣言・辛口フレーバーなし（境界: のりつぎでも順走なら辛口宣言は出さない）", () => {
  assert.deepEqual(gateSceneFor(true, "forward"), { kind: "transfer", showReverseFlavor: false });
});
test("gateSceneFor: パック本来の国・reverse はentry宣言のまま辛口フレーバーが付く（境界: kindとフレーバーは独立した軸）", () => {
  assert.deepEqual(gateSceneFor(false, "reverse"), { kind: "entry", showReverseFlavor: true });
});
test("gateSceneFor: のりつぎ国・reverse はtransfer宣言＋辛口フレーバー（§4「のりつぎ国は再訪扱い」で両方起こりうる）", () => {
  assert.deepEqual(gateSceneFor(true, "reverse"), { kind: "transfer", showReverseFlavor: true });
});
