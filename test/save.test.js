...
test("emptySaveV2: 初回起動はp1(voice:false)/p2(voice:true)の空プロフィール2枠", () => {
  ...
  for (const pid of ["p1", "p2"]) {
    ...
    assert.deepEqual(p.passport, { stamps: {}, bonus: [], routes: [] });
  }
});
  ...
  const full = {
    ...
    profiles: {
      p1: {
        ...
        passport: { stamps: { JPN: { dates: ["2026-07-18"], gold: false } }, bonus: ["BONUS-t1-1"], routes: [{ tripId: "t1-1", ids: ["JPN", "AUS"], date: "2026-07-18" }] },
      },
      p2: {
...
test("mergeSaveV2: 一部フィールドが欠けたv2でも欠損分だけ補い他は保持する", () => {
  ...
  assert.deepEqual(merged.profiles.p1.passport, { stamps: {}, bonus: [], routes: [] });
  assert.deepEqual(merged.profiles.p2, emptySaveV2().profiles.p2);
});

test("mergeSaveV2: passportの一部フィールドだけ欠けていても他は保持し欠損分だけ空で補う", () => {
  const partial = {
    version: 2,
    activeProfile: "p1",
    profiles: {
      p1: { passport: { stamps: { JPN: { dates: ["2026-07-18"], gold: true } } } }, // bonus/routes欠損
    },
  };
  const merged = mergeSaveV2(partial);
  assert.deepEqual(merged.profiles.p1.passport, {
    stamps: { JPN: { dates: ["2026-07-18"], gold: true } }, bonus: [], routes: [],
  });
});
